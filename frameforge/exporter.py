...
class ExportSettings:
    ...
    def from_scene(cls, scene, preferences=None):
        ...
        project = getattr(scene, "frameforge_project_name", "") or blend_name
        output_dir = getattr(scene, "frameforge_output_dir", "") or ""
        if output_dir:
        ...
        if not output_dir:
            output_dir = os.path.join(
                os.path.expanduser("~"), "FrameForge", util.sanitize_token(project)
            )
        ...
        return cls(
            ...
            project=project,
            resolution_key=getattr(scene, "frameforge_resolution", presets.RESOLUTION_SCENE),
            export_first=bool(getattr(scene, "frameforge_export_first", True)),
            export_last=bool(getattr(scene, "frameforge_export_last", True)),
            mid_count=int(getattr(scene, "frameforge_mid_count", 0)),
            export_depth=bool(getattr(scene, "frameforge_export_depth", False)),
            export_normal=bool(getattr(scene, "frameforge_export_normal", False)),
            depth_auto=bool(getattr(scene, "frameforge_depth_auto", True)),
            depth_near=float(getattr(scene, "frameforge_depth_near", 0.0)),
            depth_far=float(getattr(scene, "frameforge_depth_far", 0.0)),
            export_preview=bool(getattr(scene, "frameforge_export_preview", False)),
            preview_quality=int(getattr(scene, "frameforge_preview_quality", 50)),
            ...
            overwrite=bool(getattr(scene, "frameforge_overwrite", True)),
        )


def shot_names(settings, camera):
    project = util.sanitize_token(settings.project, "project")
    override = getattr(camera, "frameforge_shot_name", "") or ""
...
def preset_for_camera(scene, camera):
    override = getattr(camera, "frameforge_shot_preset", "USE_SCENE")
    if override and override != "USE_SCENE":
        return override
    return getattr(scene, "frameforge_preset", "dolly_in")
