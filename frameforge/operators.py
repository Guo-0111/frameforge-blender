...
def active_camera(context):
    scene = context.scene
    camera = getattr(scene, "frameforge_camera", None)
...
def _set_status(scene, text, report=""):
    scene.frameforge_status = text
    if report:
        scene.frameforge_report = report


class FRAMEFORGE_OT_apply_move(Operator):
    bl_idname = "frameforge.apply_move"
    ...
    def execute(self, context):
        ...
        preset_id = getattr(scene, "frameforge_preset", "dolly_in")
        ...
        try:
            result = motion.bake_camera_move(
                ...
                camera,
                preset_id,
                strength=float(getattr(scene, "frameforge_strength", 1.0)),
                easing=getattr(scene, "frameforge_easing", presets.EASING_IN_OUT),
                step=int(getattr(scene, "frameforge_keyframe_step", 1)),
                clear_animation=bool(getattr(scene, "frameforge_clear_animation", True)),
            )
...
class FRAMEFORGE_OT_export_shot(Operator):
    bl_idname = "frameforge.export_shot"
    ...
    def execute(self, context):
        ...
        scene.frameforge_last_output_dir = manifest.get("output_dir", "")
...
class FRAMEFORGE_OT_batch_export(Operator):
    bl_idname = "frameforge.batch_export"
    ...
    def execute(self, context):
        ...
        if variant.IS_LITE:
            self.report(
                {"ERROR"}, "Batch export is part of the full build of FrameForge."
            )
            return {"CANCELLED"}
        ...
        scene.frameforge_last_output_dir = settings.output_dir
        _set_status(
            scene,
...
class FRAMEFORGE_OT_preview_prompt(Operator):
    bl_idname = "frameforge.preview_prompt"
    ...
    def execute(self, context):
        ...
        text = presets.build_prompt(
            preset_id,
            float(getattr(scene, "frameforge_strength", 1.0)),
            exporter.scene_fps(scene),
            int(scene.frame_end) - int(scene.frame_start) + 1,
            has_first_last=bool(getattr(scene, "frameforge_export_first", True))
            and bool(getattr(scene, "frameforge_export_last", True)),
            has_depth=bool(getattr(scene, "frameforge_export_depth", False)),
            has_normal=bool(getattr(scene, "frameforge_export_normal", False)),
        )

        block_name = "FrameForge Prompt"
...
class FRAMEFORGE_OT_open_output_dir(Operator):
    bl_idname = "frameforge.open_output_dir"
    ...
    def execute(self, context):
        scene = context.scene
        directory = scene.frameforge_last_output_dir or ""
        if not directory:
            settings = exporter.ExportSettings.from_scene(scene, addon_preferences(context))
...
class FRAMEFORGE_OT_reset_defaults(Operator):
    bl_idname = "frameforge.reset_defaults"
    bl_label = "Reset FrameForge Settings"
    bl_description = "Restore every FrameForge setting in this scene to its default"
    bl_options = {"REGISTER", "UNDO"}

    DEFAULTS = (
        ("frameforge_preset", "dolly_in"),
        ("frameforge_strength", 1.0),
        ("frameforge_easing", presets.EASING_IN_OUT),
        ("frameforge_keyframe_step", 1),
        ("frameforge_clear_animation", True),
        ("frameforge_resolution", presets.RESOLUTION_SCENE),
        ("frameforge_export_first", True),
        ("frameforge_export_last", True),
        ("frameforge_mid_count", 0),
        ("frameforge_export_depth", False),
        ("frameforge_export_normal", False),
        ("frameforge_depth_auto", True),
        ("frameforge_depth_near", 0.0),
        ("frameforge_depth_far", 0.0),
        ("frameforge_export_preview", False),
        ("frameforge_preview_quality", 50),
        ("frameforge_overwrite", True),
    )
        ...
        for name, value in self.DEFAULTS:
            if hasattr(scene, name):
                try:
                    ...
                    pass
        _set_status(scene, "FrameForge settings reset to defaults.")
        self.report({"INFO"}, "FrameForge settings reset to defaults.")
        return {"FINISHED"}


CLASSES = (
    FRAMEFORGE_OT_apply_move,
    FRAMEFORGE_OT_export_shot,
    FRAMEFORGE_OT_batch_export,
    FRAMEFORGE_OT_preview_prompt,
    FRAMEFORGE_OT_open_output_dir,
    FRAMEFORGE_OT_reset_defaults,
)
