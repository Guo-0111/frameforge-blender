...
FrameForge ships its own ffmpeg build so that a buyer never has to install
...
def candidates(explicit_path=""):
    ...
    env_path = os.environ.get("FRAMEFORGE_FFMPEG")
    if env_path:
...
def missing_ffmpeg_help():
    ...
    if sys.platform.startswith("win"):
        return (
            "No usable ffmpeg was found. FrameForge ships one for Windows, so "
            "reinstalling the add-on usually fixes this, or point at your own copy "
            "in the add-on preferences."
        )
    if sys.platform == "darwin":
        return (
            "No usable ffmpeg was found. FrameForge only bundles ffmpeg for Windows. "
