"""FrameForge - turn a Blender scene into AI-video-ready camera moves and passes.

FrameForge is deliberately offline: it renders with Blender, encodes with a
...
def _sync_scene(scene):
    if scene is None:
        return
    try:
        if not getattr(scene, "frameforge_project_name", ""):
            scene.frameforge_project_name = _default_project_name()
    except Exception:
        pass
